...
const getImg: Intermediary<ImageIOpts> = async function (
	opts,
	data,
	_gradients,
	pipeline,
	...
	const values = Object
		.fromEntries(
			pipeline.datafields
				...
				.map(
					(v) => [v.perConfig.imagePicker?.useFor, data[v.fieldName]],
				),
		) as { [K in ImagePickerPerConf["useFor"]]: number };
...
const seenImages: { [x: string]: string[] | undefined } = {};
