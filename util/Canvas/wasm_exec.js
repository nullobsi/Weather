			...
			global.fs = {
				...
				openSync (path, flags, mode) {
					...
					if (typeof flags == "number") {
						...
						if (trunc == 1) flags.truncate = true;
						if (create == 1) flags.create = true;
				...
				fstatSync(fd) {
					//require("fs").stat
					let status = Deno.statSync(mappings[fd]);
					if (status.atime)
						status.atimeMs = status.atime.getTime();
					if (status.mtime) {
						status.mtimeMs = status.mtime.getTime();
					}

					if (status.birthtime) {
						status.birthtimeMs = status.birthtime.getTime();
					}
					if (status.isDirectory) {
						status.isDirectory = () => true;
					}
					else status.isDirectory = () => false;
					if (status.dev == null) {
						status.dev = 0;
					}
					if (status.ino == null) {
			...
			this.importObject = {
				go: {
					...
					"runtime.wasmWrite": (sp) => {
						...
						Deno.writeSync(fd, new Uint8Array(this._inst.exports.mem.buffer, p, n))
						//fs.writeSync(fd, );
					},
